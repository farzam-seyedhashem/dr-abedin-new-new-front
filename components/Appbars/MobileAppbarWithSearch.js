...
export default function MobileAppbarWithSearch({
                                                   modeType,
                                                   handleChangeMode,
                                                   handleChangeColor,
                                                   colorMode
    ...
    useEffect(() => {
        ...
        if (!themeMeta) {
            ...
            isDark ? meta.content = "#16130e" : meta.content = "#fff8f3";
            document.getElementsByTagName('head')[0].appendChild(meta);
        }
        // On page load or when changing themes, best to add inline in `head` to avoid FOUC
        if (localStorage.theme === 'dark' || (!('theme' in localStorage) && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
            setIsDark(true)
            document.querySelector('meta[name="theme-color"]').setAttribute("content", "#16130e")
        } else {
            setIsDark(false)
            document.querySelector('meta[name="theme-color"]').setAttribute("content", "#fff8f3")
    ...
    const changeThemMode = () => {
        ...
        if (localStorage.getItem('theme')) {
            ...
            if (document.documentElement.classList.contains('dark')) {
                ...
                localStorage.setItem('theme', 'light');
                setIsDark(false)
                document.querySelector('meta[name="theme-color"]').content = "#fffbff"
                document.querySelector('meta[name="theme-color"]').setAttribute("content", "#fff8f3")
                ...
                document.querySelector('meta[name="theme-color"]').content = "#1f1b16"
            }
        }
...
//         localStorage.removeItem('theme')
    ...
    useEffect(() => {
        if (isSearchDialogOpen) {
            ...
            isDark ? document.querySelector('meta[name="theme-color"]').setAttribute("content", "#2e2924") : document.querySelector('meta[name="theme-color"]').setAttribute("content", "#f0e7de")
        } else {
            isDark ? document.querySelector('meta[name="theme-color"]').setAttribute("content", "#16130e") : document.querySelector('meta[name="theme-color"]').setAttribute("content", "#fff8f3")
        }
    }, [isSearchDialogOpen])
    return (
        <>
            <Transition.Root show={isSearchDialogOpen} as={Fragment}>
                <Dialog as="div" className="absolute inset-0 flex z-999 overflow-hidden"
                    ...
                    <div
                        className="fixed top-0 z-999  w-full justify-center left-0 h-full">
                        <Transition.Child
                            as={Fragment}
                            enter="transition-all duration-500 "
                            enterFrom="opacity-0 h-0 rounded-[32px]"
                            enterTo="opacity-100 h-full rounded-[0px]"
                            leave="transition-all duration-500 "
                            leaveFrom="opacity-100 h-full rounded-[0px]"
                            leaveTo="opacity-0 h-0 rounded-[32px]"
                        >
                            <div
                                className={"bg-surface-container-high-light  dark:bg-surface-container-high-dark w-full relative  h-full  "}>
                                <div
                                    className={"border-b flex border-outline-light dark:border-outline-dark px-2 h-[72px] items-center"}>
                                    <IconButton onClick={() => setIsSearchDialogOpen(false)}
                                                className={"text-on-surface-light dark:text-on-surface-dark"}>
                                        arrow_back
                                    </IconButton>
                                    <input autoFocus onChange={(e) => setSearchText(e.target.value)}
                                           placeholder={"Search ..."}
                                           className={"text-body-large h-[48px] flex-1 placeholder-on-surface-variant-light dark:placeholder-on-surface-variant-dark text-on-surface-light dark:text-on-surface-dark px-2 bg-transparent border-0 focus:ring-0 outline-none"}/>
            ...
            <FullDialog isOpen={isOpenSetting} setIsOpen={setIsOpenSetting}>
                <DialogHead onClose={()=>setIsOpenSetting(false)} title={"Setting"} isFull/>
                <DialogContent isFull>
                    <SettingList modeType={modeType}
            ...
            <div
                className={`${isSearchDialogOpen ? "absolute w-full p-0 z-999" : "sticky px-2"} transition-all duration-700 h-[72px]  items-center bg-surface-light dark:bg-surface-dark  top-0 w-full z-999 md:hidden flex`}>
                <div
                    className={`${isSearchDialogOpen ? "rounded-none h-[72px]" : "rounded-full h-[56px]"} transition-all duration-700  bg-surface-container-high-light dark:bg-surface-container-high-dark px-2  flex  items-center  w-full `}>
                    {isSearchDialogOpen ? <IconButton onClick={() => setIsSearchDialogOpen(false)}
                                                      className={"text-on-surface-light dark:text-on-surface-dark"}>
                            arrow_back
                        </IconButton> :
                        <Icon type={"outline"} className={"justify-center flex items-center w-10 h-10 text-on-surface-light dark:!text-on-surface-dark"}
                    ...
                    <input onClick={() => {
                        setIsSearchDialogOpen(true)
                           ...
                           className={"flex-1 px-2 text-on-surface-light dark:text-on-surface-dark placeholder-on-surface-variant-light dark:placeholder-on-surface-variant-dark bg-transparent border-0 focus:ring-0 outline-none"}
                           type={"text"}/>
                    <div className={"flex space-x-2 items-center"}>
                        <button onClick={()=>setIsOpenSetting(true)}
                                ...
                                <Icon
                                    className={`material-symbols-outlined group-hover:font-vs-[1_400_0_24] font-vs-[0_400_0_24] text-on-surface-variant-light dark:text-on-surface-variant-dark`}
                                    type={"outline"}>
                                    settings
                                </Icon>
            ...
            <Transition.Root show={open} as={Fragment}>
                <Dialog onTouchMove={() => {
                    setOpen(false)
                }} as="div" className="absolute inset-0 flex z-999 overflow-hidden"
                        onClose={setOpen}>
