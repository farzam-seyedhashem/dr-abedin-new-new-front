...
export default function FilterPriceModal({filterKey, title, filters}) {
    ...
    const handleFilter = () => {
        ...
        //     query += (index !== 0 ? "," : "") + item
        // )
        if (parseInt(filterSelected[0]) === parseInt(filters[0]) && parseInt(filterSelected[1]) === parseInt(filters[1])){
        ...
        <>
            ...
            <StandardDialog setIsOpen={setIsOpen} isOpen={isOpen}>
                ...
                <DialogContent className={"w-[400px] max-h-[400px] overflow-scroll"}>
                    ...
                    <SliderInput maxValue={parseInt(filterSelected[1])} minValue={parseInt(filterSelected[0])}
                                 min={parseInt(filters[0])} max={parseInt(filters[1])}
